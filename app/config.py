...
class Settings(BaseSettings):
    ...
    # Порог альфы, ниже которого пиксель заведомо фон
    ...
    # Ядро детали: сегментатор отдаёт мягкую тень полупрозрачными пикселями
    # (альфа ~20-90), поэтому деталью считаем только уверенно непрозрачное.
    alpha_core_threshold: int = 160
    # Кривая жёсткости альфы: ниже low -> прозрачно, выше high -> непрозрачно.
    # Тень попадает под low и исчезает, сглаживание края сохраняется.
    alpha_soft_low: int = 90
    alpha_soft_high: int = 160
    # Полупрозрачность разрешена только в полосе такой ширины вокруг ядра
    edge_band_px: int = 3
