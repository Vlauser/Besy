      ...
      ? history.map((video) => `
        <div class="card" data-id="${video.id}">
          ...
          <div class="row">
            ...
            <button class="btn btn-ghost" data-remove="${video.id}" title="Убрать из истории">✕</button>
          </div>
        </div>`).join('')
      : '<div class="empty" style="grid-column:1/-1"><div class="empty-icon">🕘</div>История пока пуста</div>';
  }
