...
| `callback_query` | кнопки «Одобрить»/«Отклонить» под фото и селфи в чате модератора |
...
Всё, что уходит человеку, бот тут же присылает модератору в Telegram —
фото или селфи с кнопками «Одобрить» и «Отклонить». Без этого «на
проверке» длится столько, сколько пройдёт до того, как кто-то догадается
открыть админку. Решение из чата и решение через `/admin/*` идут одним
кодом (`services/review.py`) — иначе два пути разъедутся именно там, где
решается, увидит ли кто-то чужое фото. Права проверяются на сервере:
`callback_data` видна в клиенте и подделывается.
